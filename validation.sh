...
python3 -m py_compile $runFolder/reward_function.py
rm -r $runFolder/__pycache__
if [ $? -eq 1 ]; then echo "error in reward_function.py" && exit 1; fi
jq -e < $runFolder/model_metadata.json
if [ $? -eq 4 ]; then echo "error in model_metadata.json" && exit 1; fi
jq -e < $runFolder/hyperparameters.json
if [ $? -eq 4 ]; then echo "error in hyperparameters.hson" && exit 1; fi

#track exists
source $runFolder/run.env
